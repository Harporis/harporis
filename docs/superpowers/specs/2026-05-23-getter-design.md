...
Harporis — система поиска секретов в git-репозиториях. MVP состоит из трёх сервисов и одной шины (NATS):
...
CLI / API gateway
       ...
       │ publish ScanRequest
       ▼
NATS  harporis.scans.requests  (durable, queue group: getter-pool)
       │
       ▼
Getter(s)  ──publish chunks──▶  harporis.chunks.<scan_id>
                                       │ pull (queue group: validator-pool)
                                       ▼
                               Validator(s)
                                       │
                                       │ publish findings
                                       ▼
                              harporis.findings.<scan_id>
                                       │ pull (queue group: writer-pool)
                                       ▼
                                  Writer(s)
                                       │
                                       ▼
                              persist (file / SQL / S3 — per scan)
```
...
Корреляция через `scan_id` (UUID, end-to-end).

### Deployment topology

- Каждый сервис — отдельный Go-модуль (свой `go.mod`) и отдельный бинарник (свой `main.go`).
- Деплой: N независимых процессов (например, 10 геттеров + 5 валидаторов + 3 райтера).
- Все инстансы любого тира подключены к одному NATS-кластеру, подписаны на свой subject как **queue group** → NATS сам распределяет сообщения между инстансами тира (one-message-one-consumer).
- Авто-scale-up/down: добавили геттер → он сам присоединился к group и начал брать работу. Никакого LB-инфра не требуется.
...
```
          ...
          │ (JetStream, durable)     │   NATS отдаёт ScanRequest ровно одному
          └──────────┬───────────────┘   геттеру в группе → ACK → удаление
                     │
       ┌─────────────┼─────────────┐
       ▼             ▼             ▼
   Getter #1     Getter #2  ...  Getter #N    (N независимых процессов)
       │             │             │
       │ публикуют в свой scan-scoped subject:
       ▼             ▼             ▼
   harporis.chunks.<scan_id>     (один writer на scan, валидаторы pull'ят)
   harporis.status.<scan_id>     (события lifecycle)

Параллельно:
   harporis.scans.cancel         queue group: getter-pool
                                  Каждый геттер слушает; если scan_id "его" —
                                  отменяет, иначе игнорирует.
```

### Внутри одного инстанса геттера на один скан
...
**Горизонтальное масштабирование:** N геттер-процессов в queue group `getter-pool` берут разные ScanRequest. Внутри одного скана для MVP — единственный walker (не делим коммит-диапазон между walker'ами; revisit если профайл покажет узкое горло).

## 3. ScanRequest и типы скана

ScanRequest публикуется клиентом (API gateway / CLI через gateway) в `harporis.scans.requests`. Любой геттер из пула может его подхватить. Идемпотентность через `scan_id` (UUID, генерит клиент).

```proto
message ScanRequest {
  string         scan_id   = 1;  // UUID; если уже идёт — pickup'нувший геттер отвечает NACK с reason="ALREADY_EXISTS"
  Source         source    = 2;
  ScanType       type      = 3;
  ScanRange      range     = 4;
...
## 7. gRPC сервис (узкая роль)

Основной control plane — **NATS** (см. секцию 8). gRPC оставлен **только для двух вещей**:

```proto
service GetterService {
  // Liveness/readiness probe для k8s/systemd/healthcheck-ов.
  rpc Health (HealthRequest) returns (HealthResponse);

  // Local-dev / unit-test bypass: запустить скан напрямую без NATS.
  // В production не используется; за фичей закрыть config флагом `grpc.allow_local_start: false`.
  rpc StartScanLocal (ScanRequest) returns (ScanResponse);
}
```

**Почему не gRPC для StartScan в проде:** для горизонтального scale-out нужен либо external LB поверх gRPC (k8s svc, traefik), либо broker. NATS уже в стеке как broker — используем его. Это убирает требование к LB-инфре и автоматически даёт failover (request redeliver к другому геттеру при не-ACK).

`StartScanLocal` — backdoor для разработки. Например `getter --config dev.yaml --grpc-port 50051` + `grpcurl ... StartScanLocal` без поднятия NATS. В проде выключен.

**Идемпотентность через NATS:** ScanRequest имеет `scan_id`. Геттер при pickup'е проверяет свою local active-scans map; если уже активен — NACK на NATS с reason="ALREADY_EXISTS". В пуле геттеров маловероятен race (NATS отдаёт сообщение только одному), но защита от ретрая клиента нужна.

## 8. NATS JetStream (primary control + data plane)

### Subjects и queue groups

| Subject                        | Кто publish'ит        | Кто consume'ит           | Queue group     |
|--------------------------------|-----------------------|--------------------------|-----------------|
| `harporis.scans.requests`      | API gateway / CLI     | Getter pool              | `getter-pool`   |
| `harporis.scans.cancel`        | API gateway / CLI     | Getter pool (broadcast)  | `getter-cancel-*` (уникальная per-instance subscription чтобы все получили) |
| `harporis.chunks.<scan_id>`    | Геттер (scan owner)   | Validator pool           | `validator-pool`|
| `harporis.status.<scan_id>`    | Геттер (scan owner)   | API gateway, Writer (для метаданных) | (broadcast)|
| `harporis.findings.<scan_id>`  | Validator             | Writer pool              | `writer-pool`   |

**Зачем разные подходы для requests/cancel:**
- **requests:** один request — один обработчик (queue group `getter-pool`)
- **cancel:** все геттеры должны получить (broadcast), потому что клиент не знает, какой инстанс держит данный scan_id. Реализация: каждый геттер делает уникальную subscription (например с unique consumer name на основе host:pid), внутри сравнивает scan_id со своими активными.

### Streams

- `HARPORIS_REQUESTS` — wildcard `harporis.scans.>`
...
Streams создаются геттером на старте (idempotent — `AddStream` с if-not-exists). Каждый сервис может ensure'ить нужные ему streams, дубль не страшен.

### Delivery
- At-least-once с ACK
- `ack_wait`: 30s для chunks, 60s для requests (геттер может медленно валидировать ScanRequest при клонировании remote)
- Валидатор идемпотентен. Writer дедупит findings по `(scan_id, blob_sha, line_number, secret_type)`.

### Retention
- TTL 24h на сообщениях (после ACK удаляются раньше).
- `harporis.scans.requests`: храним до ACK от getter pool (либо max-deliver-attempts=3 → DLQ).

### Сериализация
Protobuf на NATS и gRPC.
...
**Cancellation:** клиент публикует `CancelScanRequest{scan_id}` в `harporis.scans.cancel`. Каждый геттер слушает (broadcast subscription). Тот, у кого scan_id в active-scans → close scan `context.Context` → walker и workers видят `ctx.Done()` → flush текущего чанка → emit EOS marker → exit. cat-file subprocesses kill'аются. Остальные геттеры игнорируют.
...
grpc:
  port: 50051
  allow_local_start: false   # production: false; dev/test: true (открывает StartScanLocal)
...
nats:
  ...
  consumer:
    ...
    max_in_flight_scans: 4    # сколько ScanRequest одновременно держит этот геттер
...
```
services/getter/
├── cmd/getter/main.go        # entrypoint: config load, runtime tuning, NATS connect, subscribe, gRPC Health server
├── config/getter.yaml
├── internal/
│   ├── config/               # struct, parser, validation
│   ├── grpc/                 # gRPC server: Health + (optional) StartScanLocal
│   ├── scan/                 # ScanContext, state machine, active-scans registry
│   ├── git/
...
│   │   └── gitattributes.go
│   ├── nats/
│   │   ├── connect.go        # NATS/JetStream connection
│   │   ├── streams.go        # ensure streams on startup
│   │   ├── requests.go       # subscribe queue group getter-pool, pickup ScanRequest
│   │   ├── cancel.go         # broadcast subscription for cancel
│   │   ├── publisher.go      # publish chunks + status с retry
│   │   └── status.go         # status event emit helpers
│   ├── resource/
│   │   ├── runtime.go        # GOMAXPROCS, GOMEMLIMIT setup
│   │   └── pools.go          # sync.Pool для буферов
│   └── metrics/              # Prometheus exposition
└── go.mod                    # модуль github.com/Harporis/harporis/services/getter
```
