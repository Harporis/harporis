...
Управлять getter'ом удобно через `harporis` CLI — отдельный сервис в [`services/cli/`](../cli/), который ставится на хост и говорит с NATS.
...
- **Go 1.26+** на хосте — нужен только для сборки `harporis` (бинарь крошечный, ничего не ставит в систему)
...
### 2. Поставить CLI на хосте

```bash
cd services/cli && make install   # /usr/local/bin/harporis (sudo)
# либо без sudo:
make install PREFIX=$HOME/.local  # ~/.local/bin/harporis
```
...
```bash
harporis cancel demo-1 --reason "ой, не тот репо"
```
...
`docker compose up -d --force-recreate getter` и запускай `harporis scan --local /repos/big …`.
...
```bash
# публичный репо
harporis scan --type current_state --remote-url https://github.com/foo/bar.git --wait

# с PAT
harporis scan --remote-url https://github.com/foo/private.git --remote-token ghp_xxx --wait

# по SSH-ключу
harporis scan --remote-url git@github.com:foo/bar.git --remote-ssh-key ~/.ssh/id_ed25519 --wait
```
...
# 2) сборка
cd services/getter && make build      # → bin/getter (server)
cd ../cli && make install PREFIX=$HOME/.local   # → ~/.local/bin/harporis (client)
...
harporis scan --local /tmp/demo-repo --wait
```

NATS_URL берётся из env: по умолчанию `nats://localhost:4222`. Переопределить — `NATS_URL=nats://host:port harporis ...`.
...
```
harporis scan    [flags]                    отправить ScanRequest
harporis cancel  <scan-id> [--reason "..."] отменить активный скан
harporis watch   <scan-id>                  подписаться на статус-стрим
...
| CLI говорит `usage: harporis cancel <scan-id>` | Передал scan-id, но он попал во флаг. Запусти `harporis cancel demo-1 --reason …` или с флагами впереди — CLI понимает оба порядка |
| `harporis scan` зависает на `--wait` | Подписка пытается достать status-стрим, getter не публикует. Проверь `docker compose logs getter`: вероятно, ошибка обработчика. Метрика `harporis_getter_errors_total` покажет тип |
